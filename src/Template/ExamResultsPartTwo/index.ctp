                                        ...
                                        <div class="row">  
                                         ...
                                         <div class="row">  
                                            <div  class="col-md-6">     
                                              ...
                                              <div  class="col-md-6">     
                                                <div class="form-group">
                                                    <select class="form-control" id="exam_type_include" data-placeholder="Select Exam Type" style="width: 100%;">
                            ...
                            <table id="resulttable"  class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="5%">First Exam(MM)</th>
                                        <th width="5%">First Exam(PM)</th>
                                        <th width="5%">First Exam(OM)</th>
                                        
                                        <th width="5%">Second Exam(MM)</th>
                                        <th width="5%">Second Exam(PM)</th>
                                        
                                        <th width="5%">Total(MM)</th>
                                        <th width="5%">Total(PM)</th>
                                        
                                        <th width="5%">Test Marks</th>
                                        <th width="5%">Obtain Marks</th>
                                        <th width="10%">Total Obtain Marks</th>
...
<script>
    ...
    var tabltint = function () {
        ...
        var theTable = $('#userstable').DataTable({
                ...
                "ajax": {
                    type: "POST",
                    url: "<?php echo $this->Url->build(['controller' => 'ExamResultsPartTwo', 'action' => 'getbysearch']); ?>",
                    dataType: 'json',
                    cache: false,
                    async: false,
                    "data": function ( d ) {
    ...
    function getStudents() {
        ...
        var exam_type_include = $('#exam_type_include option:selected').val();
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo $this->Url->build(['controller' => 'ExamResultsPartTwo', 'action' => 'getStudentIfo']); ?>",
            dataType: 'json',
            cache: false,
            async: false,
            data: {cc: cc,exam_type:exam_type,exam_type_include:exam_type_include},
            success: function (data) {
                ...
                if (result[0] === "Success") {
                    ...
                    for (var x = 0; x < edata.length; x++) {
                        ...
                        mhtml += "<td id='fmm"+edata[x]['subject_id']+"'>" + edata[x]['fmm'] + "</td>";
                        mhtml += "<td id='fpm"+edata[x]['subject_id']+"'>" + edata[x]['fpm'] + "</td>";
                        mhtml += "<td id='fom"+edata[x]['subject_id']+"' style='font-weight:bold;color:blue;'>" + edata[x]['fom'] + "</td>";
                        
                        mhtml += "<td id='smm"+edata[x]['subject_id']+"'>" + edata[x]['max_marks'] + "</td>";
                        mhtml += "<td id='spm"+edata[x]['subject_id']+"'>" + edata[x]['min_marks'] + "</td>";
                        
                        var TMM = 0;
                        var TMM2 = 0;
                        if($.isNumeric(edata[x]['max_marks']) && $.isNumeric(edata[x]['fmm'])){
                            TMM = parseFloat(edata[x]['max_marks']) + parseFloat(edata[x]['fmm']);
                        }
                        var TMP = 0;
                        if($.isNumeric(edata[x]['min_marks']) && $.isNumeric(edata[x]['fpm'])){
                            TMP = parseFloat(edata[x]['min_marks']) + parseFloat(edata[x]['fpm']);
                        }
                        
                        mhtml += "<td id='mm"+[edata[x]['subject_id']]+"' style='font-weight:bold;color:green;'>" + TMM + "</td>";
                        mhtml += "<td id='pm"+edata[x]['subject_id']+"' style='font-weight:bold;color:red;'>" + TMP + "</td>";
                        
                        mhtml += "<td><input type='text' id='test_id"+edata[x]['subject_id']+"' onkeyup='calc_value("+edata[x]['subject_id']+");' class='form-control' style='height:25px;width:100px;' value='0' /></td>";
                        mhtml += "<td><input type='text' id='obtain_id"+edata[x]['subject_id']+"' onkeyup='calc_value("+edata[x]['subject_id']+");' class='form-control' style='height:25px;width:100px;' value='0' /></td>";
                        mhtml += "<td><input type='text' id='total_id"+edata[x]['subject_id']+"' disabled=true class='form-control' style='height:25px;width:100px;' value='"+edata[x]['fom']+"' /></td>";
                        mhtml += "<td class='hidden'>" + edata[x]['order_id'] + "</td>";
                         
                        mhtml += '</tr>';
                        if($.isNumeric(edata[x]['max_marks']) && $.isNumeric(edata[x]['fmm'])){
                            TM += parseFloat(edata[x]['max_marks']) + parseFloat(edata[x]['fmm']);
                        }
    ...
    function add_marks() {
            ...
            if (TableData.length > 0) {
               imageOverlay('#resulttable', 'show');
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo $this->Url->build(['controller' => 'ExamResultsPartTwo', 'action' => 'add']); ?>/"+id_exam,
                    dataType: 'json',
    ...
    function storeFeeTblValues(){
        ...
        $('#resulttable tr').each(function (row, tr) {
            TableData[row] = {
                ...
                ,"fmax": $(tr).find('td:eq(2)').text()
                ,"fmin": $(tr).find('td:eq(3)').text()
                ,"fom": $(tr).find('td:eq(4)').text()
                
                ,"max": $(tr).find('td:eq(5)').text()
                ,"min": $(tr).find('td:eq(6)').text()
                
                ,"tmm": $(tr).find('td:eq(7)').text()
                ,"tpm": $(tr).find('td:eq(8)').text()
                
                ,"test_marks": $(tr).find('td:eq(9)>input').val()
                ,"obtain_marks": $(tr).find('td:eq(10)>input').val()
                ,"total_obtain_marks": $(tr).find('td:eq(11)>input').val()
    ...
    function calc_value(id){
    
        var fom = parseFloat($('#fom'+id).text()) !== '' ? parseFloat($('#fom'+id).text()) : 0;
        var test_marks = parseFloat($('#test_id'+id).val()) !== '' ? parseFloat($('#test_id'+id).val()) : 0;
        var obtain_marks =  parseFloat($('#obtain_id'+id).val()) !== '' ? parseFloat($('#obtain_id'+id).val()) : 0;
     
        var pm =  parseFloat($('#pm'+id).text()) !== '' ? parseFloat($('#pm'+id).text()) : 0;
        var mm =  parseFloat($('#mm'+id).text()) !== '' ? parseFloat($('#mm'+id).text()) : 0;
        var tob =  parseFloat($('#total_id'+id).val()) !== '' ? parseFloat($('#total_id'+id).val()) : 0;
       ...
       var total_om = parseFloat(fom + tom + om);
       
       $('#total_id'+id).val(parseFloat(total_om).toFixed(2));
   
        if (parseFloat(total_om) > parseFloat(mm)) {
            $('#total_id'+id).css({'background-color':'red','color':'white'});
            toastr["error"]("Sorry! Obtain marks can not be greater then total marks,Please check entered data.");
        }
        else if(parseFloat(total_om) < parseFloat(pm)){
            $('#total_id'+id).css({'background-color':'orange','color':'white'});
        }
        else{
            $('#total_id'+id).css({'background-color':'white','color':'black'});
        }
        calc_value2();
    }
    function checkEmpty(){
        $('#resulttable tbody tr').each(function (row, tr) {
            parseFloat($(tr).find('td:eq(9)>input').val()) === "" ? $(tr).find('td:eq(9)>input').val() : parseFloat($(tr).find('td:eq(9)>input').val());
        });
    }
    function calc_value2(){
        var i = 0;
        var OM = 0;
        var total_obtain = 0;
        $('#resulttable tbody tr').each(function (row, tr) {
            total_obtain = parseFloat($(tr).find('td:eq(11)>input').val()) === "" ? 0 : parseFloat($(tr).find('td:eq(11)>input').val());
            OM += parseFloat(total_obtain);
            console.log(total_obtain);
        });
     //   console.log(OM);
        $('#o_marks').text(parseFloat(OM).toFixed(2));
    ...
    function checkRank(){
        ...
        $('#resulttable tbody tr').each(function (row, tr) {
         var rank = $(tr).find('td:eq(9)>input').val() === "" ? 0 : $(tr).find('td:eq(9)>input').val();
    ...
    function checkResult(){
        ...
        $('#resulttable tbody tr').each(function (row, tr) {
         ...
         var rank = $(tr).find('td:eq(10)>input').val() === "" ? 0 : $(tr).find('td:eq(10)>input').val();
           if(rank === 'Abs.' || rank === 'Abs' || rank === 'Ab' || rank === 'Absent' || rank === 'abs.' || rank === 'abs' || rank === 'ab' || rank === 'absent'){
    ...
    $('#resulttable tbody tr').each(function (row, tr) {
        ...
        var passing_marks = $(tr).find('td:eq(6)').text() === "" ? 0 : $(tr).find('td:eq(6)').text();
        var total_marks = $(tr).find('td:eq(7)').text() === "" ? 0 : $(tr).find('td:eq(7)').text();
        
        var test_marks = $(tr).find('td:eq(9)>input').val() === "" ? 0 : $(tr).find('td:eq(9)>input').val();
        var obtain_marks = $(tr).find('td:eq(10)>input').val() === "" ? 0 : $(tr).find('td:eq(10)>input').val();
        var total_obtain_marks = $(tr).find('td:eq(11)>input').val() === "" ? 0 : $(tr).find('td:eq(11)>input').val();
        ...
        if($.isNumeric(test_marks) && $.isNumeric(obtain_marks) && $.isNumeric(passing_marks)){
            ...
            var total_obtain = parseFloat(total_obtain_marks);
    ...
    function edit_record(class_id,shift_id,exam_type_id,session_id,reg_id) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo $this->Url->build(['controller' => 'ExamResultsPartTwo', 'action' => 'editStudentIfo']); ?>",
            dataType: 'json',
            cache: false,
            async: false,
            ...
            success: function (data) {
                ...
                if (result[0] === "Success") {
                    ...
                    for (var x = 0; x < edata.length; x++) {
                        ...
                        var fom = 0;
                        if($.isNumeric(edata[x]['fom'])){
                            fom = parseFloat(edata[x]['fom']);
                        }
                        var stm = 0;
                        if($.isNumeric(edata[x]['test_obtain_marks'])){
                            stm = parseFloat(edata[x]['test_obtain_marks']);
                        }
                        var som = 0;
                        if($.isNumeric(edata[x]['test_obtain_marks'])){
                            som = parseFloat(edata[x]['test_obtain_marks']);
                        }
                        ...
                        var total_obtained = parseFloat(fom) + parseFloat(stm) + parseFloat(som);
                        
                        mhtml += "<td id='mm"+[edata[x]['subject_id']]+"' style='font-weight:bold;color:green;'>" + TMM + "</td>";
                        mhtml += "<td id='pm"+edata[x]['subject_id']+"' style='font-weight:bold;color:red;'>" + TMP + "</td>";
                        
                        mhtml += "<td><input type='text' id='test_id"+edata[x]['subject_id']+"' onkeyup='calc_value("+edata[x]['subject_id']+");' class='form-control' style='height:25px;width:100px;' value='"+ edata[x]['test_obtain_marks'] +"' /></td>";
                        mhtml += "<td><input type='text' id='obtain_id"+edata[x]['subject_id']+"' onkeyup='calc_value("+edata[x]['subject_id']+");' class='form-control' style='height:25px;width:100px;' value='"+ edata[x]['obtain_marks'] +"' /></td>";
                        mhtml += "<td><input type='text' id='total_id"+edata[x]['subject_id']+"' disabled=true class='form-control' style='height:25px;width:100px;' value='"+edata[x]['total_obtain_marks']+"' /></td>";
    ...
    function printResultCard() {
        ...
        var f = 3;
        window.open("<?php echo $this->Url->build(['controller' => 'ExamTypes', 'action' => 'view']); ?>/"+ f + "/" + class_id + "/" + shift_id + "/" + exam_type_id + "/" + session_id + "/" +session );

    }
    function print_result(class_id,shift_id,exam_type_id,session_id,id_exam,session_name) {
   
       var f = 3;
       window.open("<?php echo $this->Url->build(['controller' => 'ExamTypes', 'action' => 'view']); ?>/"+ f + "/" + class_id + "/" + shift_id + "/" + exam_type_id + "/" + session_id + "/" + session_name+ "/"+id_exam  );
    }
    ...
    function adminCard() {
     ...
     function setRollNumber(){
        ...
        $.ajax({
                type: "POST",
                url: "<?php echo $this->Url->build(['controller' => 'ExamResultsPartTwo', 'action' => 'setRoll']); ?>",
                dataType: 'json',
                cache: false,
                async: false,
                data: {class_id:class_id,shift_id:shift_id,session_id:session_id,exam_type_id:exam_type_id},
