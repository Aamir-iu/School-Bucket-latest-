...
class GeneralSettingController extends AppController
{
    ...
    public function index()
    {
     ...
     public function isAuthorized($user) {
        ...
        if (in_array($action, ['index','edit','updatelogo','smssetting'])&& $this->request->session()->read('Auth.User.role_id')==1 || $this->request->session()->read('Auth.User.role_id')==2 || $this->request->session()->read('Auth.User.role_id')==3) {
            return true;
        }
