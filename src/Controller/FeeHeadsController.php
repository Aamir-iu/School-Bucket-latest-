...
class FeeHeadsController extends AppController
{
    ...
    public function isAuthorized($user) {
        ...
        if (in_array($action, ['index', 'add', 'edit', 'view','loadfeeheads','adddetails','delete'])&& $this->request->session()->read('Auth.User.role_id')==1 || $this->request->session()->read('Auth.User.role_id')==2 || $this->request->session()->read('Auth.User.role_id')==3 || $this->request->session()->read('Auth.User.role_id')==5 || $this->request->session()->read('Auth.User.role_id')==6) {
            return true;
        }
