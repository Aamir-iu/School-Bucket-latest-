...
class ExpansesController extends AppController
{
    ...
    public function isAuthorized($user) {
        ...
        if (in_array($action, ['index', 'add', 'delete', 'view','getdetails','expansereport'])&& $this->request->session()->read('Auth.User.role_id')==1 || $this->request->session()->read('Auth.User.role_id')==2 || $this->request->session()->read('Auth.User.role_id')==3) {
            return true;
        }
